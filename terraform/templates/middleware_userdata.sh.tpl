...
class ServerRegistry:
    ...
    async def register_server(self, server_name: str, host: str, port: int) -> None:
...
class MiddlewareProxy:
    ...
    async def initialize_servers(self) -> None:
        """Pre-register servers from environment variables.

        Supports two formats:
          SERVER_LIST=name:host:port,name:host:port,...
          MONITOR_SERVER_<LABEL>=name:host:port
        """
    ...
    async def connect_to_server(self, server_name: str) -> Optional[tuple]:
        """Open a TCP connection to a registered server.

        Args:
            server_name: Key in the server registry.
    ...
    async def handle_client(
        ...
        try:
            welcome = {
                ...
                "process_commands": [
                    "STOP <pid> - Stop a process by PID",
            ...
            writer.write(json.dumps({"type": "server_list", "servers": servers}).encode() + b"\n")
            await writer.drain()
            ...
            while True:
                ...
                if command.upper() == "LIST":
                    servers = await self.registry.list_servers()
                    writer.write(
                        json.dumps({"type": "server_list", "servers": servers}).encode() + b"\n"
                    )
                    await writer.drain()
